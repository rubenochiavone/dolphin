	...
	: ControlGroup(_name, _ui_name, GROUP_TYPE_STICK)
{
	for (auto& named_direction : named_directions)
		controls.emplace_back(make_unique<Input>(named_direction));

	controls.emplace_back(make_unique<Input>(_trans("Modifier")));
	settings.emplace_back(make_unique<Setting>(_trans("Radius"), default_radius, 0, 100));
	settings.emplace_back(make_unique<Setting>(_trans("Dead Zone"), 0, 0, 50));
}

ControllerEmu::Buttons::Buttons(const std::string& _name) : ControlGroup(_name, GROUP_TYPE_BUTTONS)
{
	settings.emplace_back(make_unique<Setting>(_trans("Threshold"), 0.5));
}

ControllerEmu::MixedTriggers::MixedTriggers(const std::string& _name) : ControlGroup(_name, GROUP_TYPE_MIXED_TRIGGERS)
{
	settings.emplace_back(make_unique<Setting>(_trans("Threshold"), 0.9));
}
...
{
	controls.emplace_back(make_unique<Input>("Left"));
	controls.emplace_back(make_unique<Input>("Right"));
...
ControllerEmu::Force::Force(const std::string& _name) : ControlGroup(_name, GROUP_TYPE_FORCE)
{
	...
	controls.emplace_back(make_unique<Input>(_trans("Up")));
	controls.emplace_back(make_unique<Input>(_trans("Down")));
	controls.emplace_back(make_unique<Input>(_trans("Left")));
	controls.emplace_back(make_unique<Input>(_trans("Right")));
	controls.emplace_back(make_unique<Input>(_trans("Forward")));
	controls.emplace_back(make_unique<Input>(_trans("Backward")));
...
ControllerEmu::Tilt::Tilt(const std::string& _name) : ControlGroup(_name, GROUP_TYPE_TILT)
{
	...
	controls.emplace_back(make_unique<Input>("Forward"));
	controls.emplace_back(make_unique<Input>("Backward"));
	controls.emplace_back(make_unique<Input>("Left"));
	controls.emplace_back(make_unique<Input>("Right"));
	...
	settings.emplace_back(make_unique<Setting>(_trans("Dead Zone"), 0, 0, 50));
	settings.emplace_back(make_unique<Setting>(_trans("Circle Stick"), 0));
	settings.emplace_back(make_unique<Setting>(_trans("Angle"), 0.9, 0, 180));
}
	...
	, m_z(0)
{
	...
	controls.emplace_back(make_unique<Input>("Forward"));
	controls.emplace_back(make_unique<Input>("Backward"));
	controls.emplace_back(make_unique<Input>(_trans("Hide")));

	settings.emplace_back(make_unique<Setting>(_trans("Center"), 0.5));
	settings.emplace_back(make_unique<Setting>(_trans("Width"), 0.5));
	settings.emplace_back(make_unique<Setting>(_trans("Height"), 0.5));
}
